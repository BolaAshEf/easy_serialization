...
typedef FromMarkupObjCallback<OBJ extends Object> = OBJ Function(
    MarkupObj markupObj);
...
final _serializationNodes = <TypeID, _SerializationNode>{
  SerializationConfig._objectConfig.typeID:
...
class _SerializationNode {
  static final objectSerializationNode =
      _SerializationNode(SerializationConfig._objectConfig);
  ...
  static _SerializationNode? _getParentNode(
      _SerializationNode createdNode, _SerializationNode nodeParamParent,
      [_SerializationNode? parent]) {
  ...
  static _SerializationNode getParentNode(_SerializationNode createdNode) =>
      _getParentNode(createdNode, _headSerializationNode) ??
      _headSerializationNode;
...
class SerializationConfig<OBJ extends Object>
    with TypeProvider<OBJ>, TypeHash<OBJ> {
  static MarkupObj _objectToSerialization<PREM extends Object>(PREM obj) => {
        _unitTypePropName: obj,
      };
  static PREM _objectFromSerialization<PREM extends Object>(MarkupObj markup) =>
      markup[_unitTypePropName];
  ...
  })  : assert(_checkType<OBJ>()),
        _toMarkupObjCallback = toMarkupObj,
        _fromMarkupObj = fromMarkupObj {
    ensureCalcTypeID();
  }
  ...
  static SerializationConfig abstract<SER_OBJ extends SerializableMixin>() {
    ...
    const err = SerializationError(
        "This type is abstract, so you must configure its children.");
  ...
  static SerializationConfig serializable<SER_OBJ extends SerializableMixin>(
      FromMarkupObjCallback<SER_OBJ> fromMarkupObj) {
    assert(_checkType<SER_OBJ>());
    return SerializationConfig<SER_OBJ>._(
      fromMarkupObj,
      null,
    );
  }
  ...
  static SerializationConfig? _getMostSpecificObjectChildrenConfig(
      Object obj, _SerializationNode nodeParamParent) {
    for (final nodeParam in nodeParamParent.children) {
      if (nodeParam.isParentToObj(obj)) {
        final specificConfig = _getMostSpecificObjectChildrenConfig(
          obj,
          nodeParam,
        );
  ...
  bool isParentTo(SerializationConfig other) => other.provType(
        <OTHER>() => provType(<MINE>() => isSubtype<OTHER, MINE>()),
      );

  bool isChildTo(SerializationConfig other) => other.provType(
        <OTHER>() => provType(<MINE>() => isSubtype<MINE, OTHER>()),
      );

  bool isParentToObj(Object obj) => provType(<MINE>() => obj is MINE);

  static bool _checkType<TYPE>() {
    return !isSubtype<TYPE?, TYPE>() &&
        TYPE != (Object) &&
...
///
/// **Note** : it is necessary to provide [OBJ] type.
class EnumSerializationConfig<OBJ extends Enum>
    extends SerializationConfig<OBJ> {
  static MarkupObj _enumToSerialization<E extends Enum>(E obj) => {
        _enumTypePropName: obj.index,
      };
  static E _enumFromSerialization<E extends Enum>(
          MarkupObj markup, List<E> enumValues) =>
      enumValues[markup[_enumTypePropName]];

  EnumSerializationConfig(
    List<OBJ> enumValues,
  ) : super._(
