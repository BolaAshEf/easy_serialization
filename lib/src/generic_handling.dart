...
sealed class _TypeNode with SerializableMixin{
  ...
  set node(_TypeNode? n){
    ...
    final canReplace = n._typeProv.provType(<N>() => _typeProv.provType(<O>() => isSubtype<O, N>(),),);
    if(canReplace){
      _node = n;
      return;
    }else{
      final isChild = n._typeProv.provType(<N>() => _typeProv.provType(<O>() => isSubtype<N, O>(),),);
      if(!isChild){
  ...
  static _TypeNode? fromMarkupByType(dynamic markup){
    ...
    if(typeClassI == null){ return null; }
  ...
  static ({bool same, _TypeNode? o1, _TypeNode? o2}) depthComparison(_TypeNode a, _TypeNode b){
    _TypeNode? n1 = a;
    _TypeNode? n2 = b;
    ...
    if(n1?.node == null && n2?.node == null){
      final o1 = n1!;
      final o2 = n2!;
      return (same: true, o1: o1, o2: o2,);
    }

    return (same: false, o1: null, o2: null,);
  }
}
...
class _ListTypeNode extends _TypeNode{
  ...
  TypeProvider get _typeProv{
    return _internalTypeProv.provType(<INT_TYPE>() => TypeProvider<List<INT_TYPE>>());
  }
  ...
  TypeProvider get _internalTypeProv{
    ...
    }else if(_node == null && _config == null){ // there is neither.
